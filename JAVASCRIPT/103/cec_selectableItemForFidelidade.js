(function() { try {
    vlocity.cardframework.registerModule.controller('labsxdVlcFidelidadeController', ['$scope', '$rootScope', 'bpService', function($scope, $rootScope, bpService) {
        console.log("CEC_DEBUG | ::cec_selectableItemForFidelidade.js version 1::");

        $scope.isArray = angular.isArray;
        $scope.isObject = angular.isObject;

        // Functions used by the initialization
        ...
        $scope.loadConfigurationData = function(index) {
            ...
            // Try different showRadioOptions
            $scope.bpTree.response.recordsInstallments[index].showRadioOptions = {
                yes: $scope.showRadioButtonByDuration($scope.bpTree.response.recordsInstallments[index],12),
                no: $scope.showRadioButtonByDuration($scope.bpTree.response.recordsInstallments[index],0)
            };
        ...
        // Set up installments for the first page loadout
        ...
        $scope.getConfigurationFidelity = function(plan, index) {
            ...
            if (!$scope.elementVIP)
                $scope.elementVIP = $scope.getElement('IPA_CEC0043_FIDSTEP')[0].control;
